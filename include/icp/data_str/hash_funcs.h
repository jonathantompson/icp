...
#include "icp/string_util/macros.h"
...
#define CONSTANT_HASH(size, str) icp::data_str::HashString(size, str)
#else
#define CONSTANT_HASH(size, str) \
  (icp::data_str::StringHash(str).m_hash) % (size - 1)
#endif

#define DYNAMIC_HASH(size, str) icp::data_str::HashString(size, str)
